...
##### Q-function #####
pi_a1 = mstp.pi(ds.dat[:, ds.iS], 1, theta_check)
pi_t = mstp.pi(ds.dat[:, ds.iS], ds.dat[:, ds.iA], theta_check)
rho_t = pi_t / ds.dat[:, ds.iP]
for t in range(1, T):
    rho_t[np.arange(t, N * T, T)] *= rho_t[np.arange(t-1, N * T, T)]
for t in range(0, T):
    rho_t[np.arange(t, N * T, T)] /= np.mean(rho_t[np.arange(t, N * T, T)])
rho_t_1 = np.insert(rho_t[0:-1], 0, 1)
rho_t_1[np.arange(0, N * T, T)] = 1

R_weighted = rho_t * dat[:, ds.iR]
pi_a1 = pi_a1.reshape(-1, 1)
rho_t = rho_t.reshape(-1, 1)
rho_t_1 = rho_t_1.reshape(-1, 1)
Phi = (
    rho_t * ds.get_phi() ## \hat{Q}
    - rho_t_1 * (1 - pi_a1) * ds.get_phi(A=-1)  ## \hat{U} with a=-1
    - rho_t_1 * pi_a1 * ds.get_phi(A=1)  ## \hat{U} with a=1
)
